...
/** Maxellian random velocity and uniform spatial distribution , perturbation with mode 8*/
void Particles3D::maxwellianWithPerturbation(Field * EMf)
{
	if (vct->getCartesian_rank() ==0){
		cout << "------------------------------------------" << endl;
		cout << "Initialise particles with perturbation on Vy, mode 8" << endl;
		cout << "------------------------------------------" << endl;
	}
  ...
  for (int k = 1; k < grid->getNZC() - 1; k++)
  {
    ...
    for (int kk = 0; kk < npcelz; kk++)
    {
      ...
      //Add perturbation in x direction
      //if(ns==0) x += 1e-3*sin(2*M_PI/Lx*x*8);

      //add perturbation on Vy
      if(ns==0) v = v + 1e-2*sin(2*M_PI/Lx*x*8);

      const double y = (jj + .5) * (dy / npcely) + grid->getYN(i, j, k);

      //if(ns==0) y += 1e-3*sin(2*M_PI/Lx*x*8);
...
void Particles3D::mover_PC_AoS(Field * EMf)
{
  ...
  #pragma omp master
  if (vct->getCartesian_rank() == 0) {
    cout << "*** PC-AoS - MOVER species " << ns << " ***" << NiterMover << " ITERATIONS   ****" << endl;
  }
...
void Particles3D::mover_PC_AoS_vec_intr(Field * EMf)
{
  ...
  #pragma omp master
  if (vct->getCartesian_rank() == 0) {
    cout << "*** PC-AoS-Vec-Intr - MOVER species " << ns << " ***" << NiterMover << " ITERATIONS   ****" << endl;
  }
  const_arr4_pfloat fieldForPcls = EMf->get_fieldForPcls();
...
void Particles3D::mover_PC_AoS_vec(Field * EMf)
{
  ...
  #pragma omp master
  if (vct->getCartesian_rank() == 0) {
    cout << "*** PC-AoS-vec - MOVER species " << ns << " ***" << NiterMover << " ITERATIONS   ****" << endl;
  }
  const_arr4_pfloat fieldForPcls = EMf->get_fieldForPcls();
